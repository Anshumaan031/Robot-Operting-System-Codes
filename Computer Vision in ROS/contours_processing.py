...
def convert_gray_to_binary(gray_image, adaptive, show):
    ...
    if adaptive:    #if adaptive is true
        binary_image = cv2.adaptiveThreshold(gray_image, 
                            255, 
                            cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                            cv2.THRESH_BINARY_INV, 5, 2)   #cv2.THRESH_BINARY_INV because we want our backgound to be black and contour to be white
    else:
...
def getContours(binary_image):      
    ...
    contours, hierarchy = cv2.findContours(binary_image.copy(), 
                                            cv2.RETR_EXTERNAL,
	                                        cv2.CHAIN_APPROX_SIMPLE)
    return contours   #this variable is a data structure containing all the contours fould in the binary image
...
def draw_contours(image, contours, image_name):
    ...
    cv2.drawContours(image, contours, index, color, thickness)    #index shows which contour to show
    cv2.imshow(image_name,image)

def process_contours(binary_image, rgb_image, contours):   #function to process various values out of the contours we found
...
def get_contour_center(contour):
    ...
    #OpenCV moments are used to describe several properties of an image, 
    #such as the intensity of an image, its centroid, the area, and information about its orientation.
    cx=-1
    cy=-1
    if (M['m00']!=0):
        cx= int(M['m10']/M['m00'])
